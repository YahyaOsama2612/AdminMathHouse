...
const SideBar = ({
  ...
  const filteredMenu = useMemo(() => {
    ...
    return menuItems.filter((item) => {
      const parentMatches = item.title
        .toLowerCase()
      ...
      >
        ...
        >
          <div className="flex items-center gap-x-4 min-w-max">
            <div className=" p-2 rounded-xl text-one">
              <img src={Favlogo} alt="Logo" className="w-6 h-6" />
            </div>
            <div
              className={`transition-all duration-300 ${!isExpanded ? "opacity-0 translate-x-10" : "opacity-100 translate-x-0"}`}
            >
              <h1 className="font-bold text-xl text-gray-800 whitespace-nowrap">
                Math House
              </h1>
              <p className="text-[10px] text-gray-400 font-medium tracking-widest uppercase text-left">
                Admin Panel
              </p>
            </div>
          </div>
        ...
        <div className="px-4 py-4 shrink-0">
          <div
            ...
            className={`relative flex items-center group cursor-pointer ${!isExpanded ? "justify-center" : ""}`}
          >
            <div
              className={`absolute left-3 transition-colors ${searchTerm ? "text-one" : "text-gray-400"} ${!isExpanded ? "static left-0" : ""}`}
            >
              <Search size={18} />
            </div>
            {isExpanded && (
        ...
        {/* Navigation List */}
        <ul
          className="flex-1 px-3 space-y-1.5 overflow-y-auto custom-scrollbar pt-2"
              ...
              <li key={item.title} className="list-none">
                {hasChildren ? (
                  <div className="flex flex-col">
                    ...
                    >
                      <span
                        className={`shrink-0 transition-transform duration-200 ${isSectionActive ? "scale-110" : ""}`}
                      >
                        {item.icon}
                      </span>
                      ...
                      {isExpanded && (
                        <>
                          <span className="font-semibold text-[14px] flex-1 text-left">
                            {item.title}
                          </span>
                          <ChevronRight
                            size={16}
        ...
        <div className="p-4 border-t border-gray-50 mt-auto shrink-0">
          <button
            ...
            className={`group flex items-center p-3 w-full rounded-xl transition-all duration-200 ${!isExpanded ? "justify-center" : "gap-x-4"} text-gray-400 hover:bg-red-50 hover:text-red-500`}
          >
            <LogOut
              size={20}
